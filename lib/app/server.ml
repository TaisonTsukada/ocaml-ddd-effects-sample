(** cohttp-eio サーバーの組み立て。
    ...
    どの Port を誰が実装するか ({!Composition_root}) も、Driver のインスタンス ([store]) も
...
(** [wrap] は {!Composition_root.handler} を部分適用したもの。cohttp-eio は接続ごとに fiber を
    fork し、 effect は fiber をまたげないので、ハンドラはリクエストごとに (callback の中で) 被せる。 *)
